...
def fuel_for_kg_mass(payload_mass: float) -> float:
    return (1 / 5) * payload_mass


def launch_fuel(payload: float) -> float:
    extra_fuel: float = 1e5

    current_fuel: float = fuel_for_kg_mass(payload)

    total_fuel: float = current_fuel

    total_mass_before: float = payload + current_fuel

    while extra_fuel > 1:
        current_fuel = fuel_for_kg_mass(total_mass_before)
        total_mass_after = payload + current_fuel
        extra_fuel = total_mass_after - total_mass_before
